...
const { WebsocketProvider, HttpProvider, IpcProvider } = require('web3-providers');
  ...
  reconnect() {
    ...
    if (this._opts.reconnect) {
      if (_.isFunction(this.provider.on)) {
        const handler = () => {
          if (this.provider) this.reconnect();
        };
...
function _createProvider(uri, opts = {}) {
  if (/^https?:\/\/.+$/.test(uri)) return new HttpProvider(uri, opts);
  if (/^wss?:\/\/.+$/.test(uri)) return new WebsocketProvider(uri, opts);
  if (!opts.net) throw new Error(`IPC transport requires 'net' option.`);
...
function createProviderURI(websocket, network, infuraKey) {
  ...
  if (websocket) return `wss://${network}.infura.io/ws/v3/${infuraKey}`;
